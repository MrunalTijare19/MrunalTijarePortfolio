...
from portfolio.models import Contact,Blogs,Intership
...
def internshipdetails(request):
    ...
    if request.method=="POST":
        ...
        check1=Intership.objects.filter(usn=fusn)
        check2=Intership.objects.filter(email=femail)
        
        if check1 or check2:
            messages.warning(request,"Your Details Present Already")
        
        query=Intership(fullname=fname,usn=fusn,email=femail,college_name=fcollege,offer_status=foffer,start_date=fstartdate,end_date=fenddate,proj_report=fprojreport)
        query.save()
